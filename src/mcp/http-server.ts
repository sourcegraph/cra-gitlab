...
export function createMCPRoutes(): Hono {
  ...
  app.post('/tools/call', async (c) => {
    try {
      ...
      switch (name) {
        case 'leave_comment':
          result = await leaveComment(validateLeaveCommentArgs(args), config, gitlabClient);
          break;

        case 'post_commit_status':
          result = await postCommitStatus(validatePostCommitStatusArgs(args), config, gitlabClient);
          break;

        case 'get_mr_info':
          result = await getMRInfo(validateGetMRInfoArgs(args), config, gitlabClient);
          break;

        case 'trigger_review':
          result = await triggerReview(validateTriggerReviewArgs(args), config, gitlabClient);
          break;
        ...
        const tools = [
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                path: { type: 'string', description: 'File path for inline comments (optional)' },
                line: { type: 'number', description: 'Line number for inline comments (optional)' },
                line_type: { type: 'string', enum: ['ADDED', 'REMOVED', 'CONTEXT'], description: 'Type of line for inline comments (optional)' },
                base_sha: { type: 'string', description: 'Base commit SHA for inline comments (optional)' },
                start_sha: { type: 'string', description: 'Start commit SHA for inline comments (optional)' },
                head_sha: { type: 'string', description: 'Head commit SHA for inline comments (optional)' },
              },
              required: ['message', 'project_id', 'mr_iid'],
            },
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                project_id: { type: 'number', description: 'GitLab project ID' },
                commit_sha: { type: 'string', description: 'Commit SHA to update status for' },
                state: { type: 'string', enum: ['success', 'failed', 'running', 'pending', 'canceled'], description: 'Status state' },
                description: { type: 'string', description: 'Status description (optional)' },
                context: { type: 'string', description: 'Status context (optional)' },
                target_url: { type: 'string', description: 'Target URL for the status (optional)' },
              },
              required: ['project_id', 'commit_sha', 'state'],
            },
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                project_id: { type: 'number', description: 'GitLab project ID' },
                mr_iid: { type: 'number', description: 'Merge request IID' },
                include_diff: { type: 'boolean', description: 'Include diff content (optional, default: false)', default: false },
              },
              required: ['project_id', 'mr_iid'],
            },
          },
          {
            ...
            inputSchema: {
              ...
              properties: {
                project_id: { type: 'number', description: 'GitLab project ID' },
                mr_iid: { type: 'number', description: 'Merge request IID' },
                commit_sha: { type: 'string', description: 'Specific commit SHA to review (optional)' },
                force: { type: 'boolean', description: 'Force re-review even if already reviewed (optional)', default: false },
              },
              required: ['project_id', 'mr_iid'],
            },
          },
