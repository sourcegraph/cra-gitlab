import { GitLabClient } from '../../gitlab/client.js';
import { Config } from '../../config.js';

export interface LeaveInlineCommentArgs {
  message: string;
  project_id: number;
  mr_iid: number;
  path: string;
  line: number;
  line_type: 'new' | 'old';
  base_sha?: string;
  start_sha?: string;
  head_sha?: string;
}

export async function leaveInlineComment(
  args: LeaveInlineCommentArgs,
  config: Config,
  gitlabClient: GitLabClient
): Promise<{ success: boolean; discussion_id?: string; note_id?: number; error?: string }> {
  try {
    const { message, project_id, mr_iid, path, line, line_type, base_sha, start_sha, head_sha } = args;

    console.log('🎯 Creating inline comment via discussions API:', { path, line, line_type });
    
    // Get SHA values if not provided
    let actualBaseSha = base_sha;
    let actualStartSha = start_sha;
    let actualHeadSha = head_sha;
    
    if (!actualBaseSha || !actualStartSha || !actualHeadSha) {
      console.log('🔍 Fetching missing SHA values from MR...');
      try {
        const mrInfo = await gitlabClient.getMRInfo(project_id, mr_iid);
        actualBaseSha = actualBaseSha || mrInfo.diff_refs?.base_sha;
        actualStartSha = actualStartSha || mrInfo.diff_refs?.start_sha;
        actualHeadSha = actualHeadSha || mrInfo.diff_refs?.head_sha;
        console.log('✅ SHA values:', { 
          base: actualBaseSha?.substring(0, 8),
          start: actualStartSha?.substring(0, 8), 
          head: actualHeadSha?.substring(0, 8)
        });
      } catch (error) {
        console.log('⚠️ Could not fetch MR info for SHAs:', error);
        throw new Error('Cannot create inline comment without valid SHA values');
      }
    }

    if (!actualBaseSha || !actualStartSha || !actualHeadSha) {
      throw new Error('Missing required SHA values for inline comment');
    }

    // Create position object for inline comments using discussions API format
    const position = {
      base_sha: actualBaseSha,
      start_sha: actualStartSha,
      head_sha: actualHeadSha,
      position_type: 'text',
      new_path: path,
      old_path: path,
    } as any;

    if (line_type === 'new') {
      position.new_line = line;
    } else {
      position.old_line = line;
    }
    
    console.log('📍 Creating inline discussion at position:', position);

    const response = await gitlabClient.createMRDiscussion(project_id, mr_iid, message, position);
    
    return {
      success: true,
      discussion_id: response.id,
      note_id: response.notes?.[0]?.id
    };
  } catch (error) {
    console.error('Failed to leave inline comment:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    };
  }
}
